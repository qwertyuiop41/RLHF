...
class GRPOTrainer():
    def __init__(self,args):
        ...
        train_data=load_dataset("parquet", data_files=train_path,split='train',streaming=True).shuffle(seed=42).take(450)
        test_data=load_dataset("parquet", data_files=test_path,split='train',streaming=True).shuffle(seed=42).take(50)
    ...
    def prepare_inputs(self,batch):
        ...
        with torch.no_grad():
            ...
            rwd_score=self.compute_reward_score(seq,attention_mask=seq_mask,completions=completions,labels=batch["labels"])
            print(f"rwd_score:{rwd_score}")
            adv=self.compute_adv(rwd_score, seq_mask)
            print(f"adv:{adv}")
    ...
    def compute_adv(self,rewards, attention_mask):
        """
        ...
        # expand to [B, T] such that each token gets same adv as its sentence

        batch_size, seq_len = attention_mask.shape

        A = A.repeat(1, attention_mask.shape[1])  # 得到 [bs, seq_len]


        # A_token = A.unsqueeze(1).expand(-1, seq_len) * attention_mask  # [B, T]
        return A
    ...
    def compute_loss(self,log_probs,old_log_probs,ref_log_probs,mask,adv):
        ...
        # kl
        kl = torch.exp(ref_log_probs - log_probs) - (ref_log_probs - log_probs) - 1  # [B, T]
        ...
        loss1=ratio*adv
        loss2 = torch.clamp(ratio, 1.0 - self.cliprange,
...
if __name__=="__main__":
    ...
    #outputs
    parser.add_argument("--output_dir", default='/HOME/sustc_yqzhang/sustc_yqzhang_1/sy/RLHF/outputs/grpo/countdown/')
