...
class PPOTrainer():
    def __init__(self,args):
        ...
        self.policy_model,self.policy_optimizer,dataloader, self.policy_lr_scheduler=createModel(pretrain_path,device=device,lora_config=lora_config,bnb_config=bnb_config)

        self.value_model,self.value_optimizer,dataloader, self.value_lr_scheduler=createModel(pretrain_path,device=device,lora_config=lora_config,bnb_config=bnb_config)

        self.reward_model=RewardModel(pretrain_path,lora_config,bnb_config=bnb_config)

        self.reward_model,_,_,_= deepspeed.initialize(model=self.reward_model)


        self.train_dataset=data_prepare(self.tokenizer,train_dataset,device)
        self.test_dataset=data_prepare(self.tokenizer,test_dataset,device)

        self.train_dataloader=dataloader(dataset=CustomDataset(train_dataset),shuffle=True,batch_size=1)
        self.test_dataloader=dataloader(dataset=CustomDataset(test_dataset),shuffle=False,batch_size=1)
...
def createModel(pretrain_path,device,lora_config=None,bnb_config=None):
    config=AutoConfig.from_pretrained(pretrain_path)
    if bnb_config:
        policy_model = AutoModelForCausalLM.from_pretrained(pretrain_path,quantization_config=bnb_config).to(device)
    else:
        policy_model = AutoModelForCausalLM.from_pretrained(pretrain_path).to(device)
    if lora_config:
        policy_model=get_peft_model(policy_model,lora_config)
    policy_model, policy_optimizer, train_dataloader, lr_scheduler = deepspeed.initialize(model=policy_model)
    deepspeed.init_distributed()

    return policy_model,policy_optimizer,train_dataloader, lr_scheduler
